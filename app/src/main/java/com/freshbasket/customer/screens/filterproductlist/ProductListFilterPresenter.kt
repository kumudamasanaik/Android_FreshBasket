...
class ProductListFilterPresenter(view: ProductListFilterContract.View?) : ProductListFilterContract.Presenter, IResponseInterface {
    private var iResponseInterface: ApiResponsePresenter = ApiResponsePresenter(this)
    var view: ProductListFilterContract.View? = view

    override fun getProductList(subCatId: String) {
        iResponseInterface.callApi(MyApplication.service.getALlProductList(ApiRequestParam.getChildProductListing(subCatId)), ApiType.PRODUCT_LIST_HEADRES)
    }
        ...
        if (response.isSuccessful && Validation.isValidObject(response)) {
            when (reqType) {
                ApiType.PRODUCT_LIST_HEADRES ->
                    view!!.setProductListApiResp(response.body() as ProductResp)
